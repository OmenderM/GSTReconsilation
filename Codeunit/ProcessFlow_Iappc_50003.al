codeunit 50003 "Process Flow"
{
    ...
    // [EventSubscriber(ObjectType::Table, Database::Customer, 'OnBeforeValidateEvent', 'Customer Posting Group', true, true)]
    // local procedure CustomerPostingGroupChangeCheck(var Rec: Record Customer)
    // var
    //     recCustomerLedgerEntry: Record "Cust. Ledger Entry";
    // begin
    //     recCustomerLedgerEntry.Reset();
    //     recCustomerLedgerEntry.SetCurrentKey("Customer No.", "Posting Date", "Currency Code");
    //     recCustomerLedgerEntry.SetRange("Customer No.", Rec."No.");
    //     if recCustomerLedgerEntry.FindFirst() then
    //         Error('Can not change customer posting group as ledger entries exist for the selected customer.');
    // end;

    // [EventSubscriber(ObjectType::Table, Database::Vendor, 'OnBeforeValidateEvent', 'Vendor Posting Group', true, true)]
    // local procedure VendorPostingGroupChangeCheck(var Rec: Record Vendor)
    // var
    //     recVendorLedgerEntry: Record "Vendor Ledger Entry";
    // begin
    //     recVendorLedgerEntry.Reset();
    //     recVendorLedgerEntry.SetCurrentKey("Vendor No.", "Posting Date", "Currency Code");
    //     recVendorLedgerEntry.SetRange("Vendor No.", Rec."No.");
    //     if recVendorLedgerEntry.FindFirst() then
    //         Error('Can not change vendor posting group as ledger entries exist for the selected vendor.');
    // end;

    // [EventSubscriber(ObjectType::Table, Database::Item, 'OnBeforeValidateEvent', 'Inventory Posting Group', true, true)]
    // local procedure ItemPostingGroupChangeCheck(var Rec: Record Item)
    // var
    //     recItemLedgerEntry: Record "Item Ledger Entry";
    // begin
    //     recItemLedgerEntry.Reset();
    //     recItemLedgerEntry.SetCurrentKey("Item No.", "Posting Date");
    //     recItemLedgerEntry.SetRange("Item No.", Rec."No.");
    //     if recItemLedgerEntry.FindFirst() then
    //         Error('Can not change inventory posting group as ledger entries exist for the selected item.');
    // end;

    // [EventSubscriber(ObjectType::Table, Database::"FA Depreciation Book", 'OnBeforeValidateEvent', 'FA Posting Group', true, true)]
    // local procedure FAPostingGroupChangeCheck(var Rec: Record "FA Depreciation Book")
    // var
    //     recFALedgerEntry: Record "FA Ledger Entry";
    // begin
    //     recFALedgerEntry.Reset();
    //     recFALedgerEntry.SetCurrentKey("FA No.", "Posting Date");
    //     recFALedgerEntry.SetRange("FA No.", Rec."FA No.");
    //     if recFALedgerEntry.FindFirst() then
    //         Error('Can not change FA posting group as ledger entries exist for the selected FA.');
    // end;
    ...
    //     /*
    //     if recUserSetup."User Responsibility" <> '' then begin
    //         if StrPos(recUserSetup."User Responsibility", '|') <> 0 then
    //             Rec."Responsibility Center" := CopyStr(recUserSetup."User Responsibility", 1, StrPos(recUserSetup."User Responsibility", '|') - 1)
    //         else
    //             Rec."Responsibility Center" := recUserSetup."User Responsibility";
    //         Rec.Modify();
    //     end;
    //     /*
    ...
    //     recPurchaseLine.SetFilter(Quantity, '<>%1', 0);
